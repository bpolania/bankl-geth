...
func testRepairWithScheme(t *testing.T, tt *rewindTest, snapshots bool, scheme string) {
	...
	chain, err := NewBlockChain(db, config, gspec, nil, engine, vm.Config{}, nil, path_to_addresses)
	if err != nil {
	...
	newChain, err := NewBlockChain(db, config, gspec, nil, engine, vm.Config{}, nil, path_to_addresses)
	if err != nil {
...
func testIssue23496(t *testing.T, scheme string) {
	...
	const path_to_addresses = "/data/addresses.txt"
	chain, err := NewBlockChain(db, DefaultCacheConfigWithScheme(scheme), gspec, nil, engine, vm.Config{}, nil, path_to_addresses)
	if err != nil {
		t.Fatalf("Failed to create chain: %v", err)
	}
