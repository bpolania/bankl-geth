...
func TestStateProcessorErrors(t *testing.T) {
	...
	{ // Tests against a 'recent' chain definition
		var (
			...
			blockchain, _  = NewBlockChain(db, nil, gspec, nil, beacon.New(ethash.NewFaker()), vm.Config{}, nil, path_to_addresses)
	...
	// ErrTxTypeNotSupported, For this, we need an older chain
	{
		var (
			...
			blockchain, _ = NewBlockChain(db, nil, gspec, nil, ethash.NewFaker(), vm.Config{}, nil, path_to_addresses)
		)
		defer blockchain.Stop()
		for i, tt := range []struct {
			txs  []*types.Transaction
